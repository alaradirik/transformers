...
def convert_efficientformerv2_checkpoint(
    checkpoint_path: Path, efficientformerv2_config_file: Path, pytorch_dump_path: Path, push_to_hub: bool
):
    orig_state_dict = torch.load(checkpoint_path, map_location="cpu")["model"]
    config = EfficientFormerV2Config.from_json_file(efficientformerv2_config_file)
    model = EfficientFormerV2ForImageClassificationWithTeacher(config)
    ...
    image_size = 256
    crop_size = 224
    processor = EfficientFormerV2ImageProcessor(
    ...
    if "s0" in model_name:
        ...
        raise ValueError(
            f"Unknown model checkpoint: {checkpoint_path}. Supported version of efficientformer are s0, s1, s2 and l"
        )
    ...
    if push_to_hub:
        ...
        model.push_to_hub(
            repo_id=f"adirik/{pytorch_dump_path}",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to EfficientFormerV2 pytorch checkpoint.",
    )
    parser.add_argument(
        "--config_file",
        default=None,
        type=str,
        required=True,
        help="The json file for EfficientFormerV2 model config.",
    )
    parser.add_argument(
    ...
    convert_efficientformerv2_checkpoint(
        checkpoint_path=args.pytorch_model_path,
        efficientformerv2_config_file=args.config_file,
