...
""" EfficientFormerV2 model configuration"""
...
EFFICIENTFORMERV2_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "snap-research/efficientformerv2-s0": (
        "https://huggingface.co/snap-research/efficientformerv2-s0/resolve/main/config.json"
    ),
}


class EfficientFormerV2Config(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of an [`EfficientFormerV2Model`]. It is used to
    instantiate an EfficientFormerV2 model according to the specified arguments, defining the model architecture.
    Instantiating a configuration with the defaults will yield a similar configuration to that of the EfficientFormerV2
    [snap-research/efficientformerv2-l](https://huggingface.co/snap-research/efficientformer-l1) architecture.
    ...
    ```python
    >>> from transformers import EfficientFormerV2Config, EfficientFormerV2Model

    >>> # Initializing a EfficientFormerV2 efficientformerv2-l style configuration
    >>> configuration = EfficientFormerConfig()

    >>> # Initializing a EfficientFormerV2Model (with random weights) from the efficientformerv2-l style configuration
    >>> model = EfficientFormerV2Model(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config
    ```"""

    model_type = "efficientformerv2"
    ...
    def __init__(
        ...
        dim: int = 448,
        expansion_ratios: List[List[int]]: [
            [4, 4, 4, 4, 4],
            [4, 4, 4, 4, 4],
